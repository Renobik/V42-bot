...
Upgrade from V4.1: Same logic, cleaner state, 5% risk locked, auto Top100 refresh, telegram_offset fix
"""
import ccxt, os, json, time, math, requests, io
...
def get_top100_symbols(exchange):
    ...
    if CMC_API_KEY:
        try:
            ...
            # filter only those available on binance
...
def detect_choch(df, swing_len):
    ...
    # Bullish CHoCH: swept low then broke high
...
def check_execution(bias_4h, df_1h, price_now):
    ...
    if dist<=PULLBACK_DIST_PCT:
        ...
        # still aligned but not pullback
...
def main():
    ...
    while True:
        ...
        for sym in symbols:
            ...
            if not DRY_RUN:
                try:
                    ...
                    # TP1 = 5% notional target -> approx 1R? Using 5% profit on margin?
        ...
        # poll telegram every 30s during sleep
