...
# Robust CHAT_ID - fix space bug from screenshot
...
def tg_send(msg, buttons=None, reply_chat=None):
    ...
    try:
        # Use reply_chat if provided (fixes CHAT_ID bug) else env CHAT_ID
        ...
        print(f"tg_send to {target} -> {r.status_code} {r.text[:200]}")
...
def webhook():
    ...
    # FIX: reply to sender, not fixed CHAT_ID - solves your bug!
