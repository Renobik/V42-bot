"""
V44.21 - HUNTER 24/7 Top200 FIXED - No json bug
"""
...
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "v44-secret-xyz123")
...
def tg_answer_callback(cb_id, text=""):
    try: 
        requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/answerCallbackQuery", json={"callback_query_id":cb_id,"text":text}, timeout=5)
    except: pass
...
def find_fvgs(candles, lookback=20):
    ...
    for i in range(1, len(candles)-1):
        ...
        if c3["low"] > c1["high"]: fvgs.append({"type":"bullish","high":c3["low"],"low":c1["high"]})
        if c3["high"] < c1["low"]: fvgs.append({"type":"bearish","high":c1["low"],"low":c3["high"]})
    return fvgs
...
def detect_choch(candles, bg_sym):
    ...
    pd_pct, _, _ = get_premium_discount(candles,50)
    ...
    for i in range(choch_idx_high+1, len(candles)-1):
        if candles[i]["close"]>last_swing_high*1.001 and candles[i-1]["close"]<=last_swing_high*1.001:
            if pd_pct<50:
                ...
                return {"side":"LONG","choch_price":last_swing_high,"ob":ob,"fvg":overlapping_fvg,"pd_pct":pd_pct,"sweep":sweep,"sweep_valid": sweep and sweep["type"]=="bullish","vol_ok":vol_ok,"htf":htf_bias,"choch_close":candles[i]["close"]}
    for i in range(choch_idx_low+1, len(candles)-1):
        if candles[i]["close"]<last_swing_low*0.999 and candles[i-1]["close"]>=last_swing_low*0.999:
            if pd_pct>50:
                ...
                if ob:
                    for f in fvgs:
                        if f["type"]=="bearish" and not (ob["high"]<f["low"] or ob["low"]>f["high"]):
                            overlapping_fvg=f; break
                return {"side":"SHORT","choch_price":last_swing_low,"ob":ob,"fvg":overlapping_fvg,"pd_pct":pd_pct,"sweep":sweep,"sweep_valid": sweep and sweep["type"]=="bearish","vol_ok":vol_ok,"htf":htf_bias,"choch_close":candles[i]["close"]}
    return None
...
def open_position(symbol, side, entry, sl, info, manual=False):
    ...
    tg_send(f"🚀 {'MANUAL ' if manual else ''}PAPER {symbol} {side} {grade} HUNTER\n{'DISC' if info['pd_pct']<50 else 'PREM'} {info['pd_pct']:.1f}% HTF {info.get('htf')} {'Sweep✅' if info.get('sweep_valid') else 'Sweep❌'} {'Vol✅' if info.get('vol_ok') else 'Vol❌'}\n{ob_txt}\n{fvg_txt}\nEntry {entry:.4f} SL {sl:.4f}", buttons)
    return True
...
def scanner_loop():
    print(f"--- V44.21 HUNTER 24/7 FIXED Top{SCAN_COUNT} ---")
    tg_send(f"🏹 V44.21 HUNTER FIXED LIVE Top{SCAN_COUNT}\n24/7 No Sleep | A+++/A+ Only\nHTF={USE_HTF_BIAS} Vol={USE_VOLUME_FILTER}")
    while True:
        try:
            top=bitget_top200()
            update_positions()
            ...
            scanned=0
            for t in top:
                if len(state["positions"])>=1 or len(state["pending"])>=5: break
                ...
                if res and res["ob"]:
                    ...
                    if is_aplus:
                        ...
                        tg_send(f"👀 {grade} CHoCH {res['side']} {ccxt} HUNTER\n{'DISC' if res['pd_pct']<50 else 'PREM'} {res['pd_pct']:.1f}% HTF {res['htf']} {'Sweep✅' if res['sweep_valid'] else 'Sweep❌'} {'Vol✅' if res['vol_ok'] else 'Vol❌'}\nBreak {res['choch_price']:.4f} -> OB50 {res['ob']['50']:.4f}\n{'FVG✅' if res['fvg'] else 'FVG❌'}", buttons)
                if scanned>=SCAN_COUNT: break
            if scanned>0:
                tg_send(f"✅ V44.21 Checked={scanned}/{SCAN_COUNT} Pend={len(state['pending'])} Pos={len(state['positions'])} Sig={state['today_signals']} Bal=${state['balance']:.2f}")
            time.sleep(60)
        except Exception as e:
            print(f"Err {e}"); import traceback; traceback.print_exc(); time.sleep(10)

@app.route('/')
def home(): return f"V44.21 HUNTER FIXED Top{SCAN_COUNT} Bal ${state['balance']:.2f} Pos {len(state['positions'])} Pend {len(state['pending'])}",200
    ...
    if "callback_query" in data:
        ...
        if cb_data.startswith("longnow:") or cb_data.startswith("shortnow:"):
            ...
            if sym in state["pending"]:
                ...
                sl = cur*(1-INIT_SL_PCT/100) if side=="LONG" else cur*(1+INIT_SL_PCT/100)
            if abs(cur-sl)/cur*100>4: sl = cur*(1-INIT_SL_PCT/100) if side=="LONG" else cur*(1+INIT_SL_PCT/100)
            ...
            sym=cb_data.split(":",1)[1]
            if sym in state["pending"]: del state["pending"][sym]; save_state(state); tg_answer_callback(cb_id,f"Skip {sym}")
        elif cb_data.startswith("close:"):
            sym=cb_data.split(":",1)[1]
            if sym in state["positions"]: state["positions"].pop(sym); save_state(state); tg_answer_callback(cb_id,f"Closed {sym}")
        return "ok",200
    if "message" not in data: return "ok",200
    txt=data["message"].get("text","").lower()
    if "status" in txt or "/status" in txt:
        msg=f"🏹 V44.21 Top{SCAN_COUNT} Bal ${state['balance']:.2f} Pos {len(state['positions'])} Pend {len(state['pending'])}\n"
        ...
        state["positions"]={}; save_state(state); tg_send("❌ Closed ALL")
    elif "/clearpending" in txt:
        state["pending"]={}; save_state(state); tg_send("Cleared pendings")
    return "ok",200
