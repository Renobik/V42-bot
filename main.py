"""
HUNTER V4.4 - PERMANENT STORAGE (Free)
- Fixes balance reset on deploy
- Uses npoint.io free JSON storage (no card, permanent)
- Fallback to local file if no NPOINT_ID
- Anti-spam preserved from V4.3
"""
...
NPOINT_ID = os.getenv("NPOINT_ID", "").strip()  # e.g. abc123 from https://api.npoint.io/abc123
LAST_UPDATE_ID = 0
LAST_TRADE_TIME = {}
GLOBAL_LAST_TRADE = 0
...
def load_from_npoint():
    ...
    try:
        ...
        if r.status_code == 200:
            data = r.json()
            print(f"💾 Loaded from npoint.io/{NPOINT_ID}: Bal ${data.get('meta',{}).get('current_balance','?')}")
            return data
    except Exception as e:
        print(f"⚠️ npoint load error: {e}")
    return None

def save_to_npoint(full_data):
    if not NPOINT_ID:
        return False
    try:
        url = f"https://api.npoint.io/{NPOINT_ID}"
        r = requests.post(url, json=full_data, timeout=10)
        print(f"💾 Saved to npoint: {r.status_code}")
        return r.status_code == 200
    except Exception as e:
        print(f"⚠️ npoint save error: {e}")
        return False
...
def telegram_listener(bot_instance):
    global LAST_UPDATE_ID, GLOBAL_LAST_TRADE
        ...
        try:
            ...
            for update in r.get("result", []):
                ...
                if text.startswith("/start") or text.startswith("/help"):
                    send_telegram(f"🦈 *V4.4 Permanent*\nBal: ${bot_instance.balance:.2f}\nNPoint: {'✅' if NPOINT_ID else '❌ not set'}\n/ping /status /trades /export")
                elif text.startswith("/ping"):
                    send_telegram(f"✅ *PONG V4.4*\nBal: ${bot_instance.balance:.2f}\nStorage: {'npoint.io ☁️' if NPOINT_ID else 'local ⚠️ resets on deploy'}")
                elif text.startswith("/status"):
                    try:
                        with open(JSON_FILE, 'r') as f:
                            j = json.load(f)
                            m = j["meta"]
                            send_telegram(f"🦈 *STATUS*\nBal: ${m['current_balance']:.2f} PnL ${m['total_pnl']:.2f}\nTrades: {m['total_trades']} WR: {m['win_rate']}%\nStorage: {'☁️ Permanent' if NPOINT_ID else '⚠️ Temporary'}")
                    except:
                    ...
                    try:
                        with open(JSON_FILE, 'r') as f:
                            ...
                            txt = "📊 *Last 5:*\n" + "\n".join([f"{t['symbol']} {t['result']} ${t['pnl_usd']:.4f}" for t in trades]) if trades else "No trades"
                            send_telegram(txt)
                    except Exception as e:
                    ...
                    try:
                        with open(JSON_FILE, 'r') as f:
                            j = json.load(f)
                            # send file as text summary
                            send_telegram(f"💾 Export: {len(j['trades'])} trades, Bal ${j['meta']['current_balance']}\nLink: https://v42-bot-2.onrender.com/trades")
                    except:
                        pass
...
def init_history():
    # 1. Try load from permanent cloud
    cloud_data = load_from_npoint()
    if cloud_data:
        # Restore local files from cloud
        try:
            with open(JSON_FILE, 'w') as f:
                json.dump(cloud_data, f, indent=2)
            with open(BALANCE_FILE, 'w') as f:
                json.dump({"current_balance": cloud_data["meta"]["current_balance"], "start_balance": cloud_data["meta"]["start_balance"], "today_start": cloud_data["meta"]["current_balance"], "today_date": START_DATE}, f, indent=2)
            print(f"✅ Restored from permanent storage: ${cloud_data['meta']['current_balance']}")
            # CSV will be rebuilt if needed
...
def log_trade_auto(symbol, side, entry, exit_price, pnl_usd, result="WIN", notify=True):
    ...
    print(f"✅ {symbol} {result} ${pnl_usd:.4f} -> ${new_bal:.2f} {'☁️' if NPOINT_ID else ''}")
...
class HunterBotV4:
    def __init__(self):
        ...
        print(f"🦈 V4.4 PERMANENT - Bal ${self.balance} Storage: {'npoint.io/'+NPOINT_ID if NPOINT_ID else 'LOCAL (resets!)'}")
        if NPOINT_ID:
            send_telegram(f"🦈 *V4.4 Permanent Started*\nBal: ${self.balance:.2f} ☁️\nStorage: Permanent (never resets)\nCooldown 15min/coin")
        else:
            send_telegram(f"🦈 *V4.4 Started - TEMP storage*\nBal: ${self.balance:.2f} ⚠️\nSet NPOINT_ID for permanent!\nCooldown 15min/coin")
    ...
    if HAS_FLASK:
        ...
        def home():
            try:
                ...
                return jsonify({"balance": bot.balance, "storage": "permanent" if NPOINT_ID else "temporary", "status": "running"})
        @app.route("/health")
        def health():
            return "OK", 200
        @app.route("/trades")
        ...
        def test_telegram():
            ok = send_telegram("✅ V4.4 Test OK")
            return jsonify({"sent": ok})
        @app.route("/debug")
        def debug_env():
            return jsonify({"balance": bot.balance, "npoint_set": bool(NPOINT_ID), "npoint_id": NPOINT_ID[:8]+"..." if NPOINT_ID else None, "storage": "permanent" if NPOINT_ID else "temporary (resets on deploy!)"})
