...
"""
V44.8 INSTANT SCAN AFTER RF - More signals, same webhook, paper trading + reports
"""
import ccxt
import pandas as pd
import time
        ...
        try:
            ...
            for sym, pos in list(state["positions"].items()):
                ...
                if not pos.get('risk_free') and pnl_pct >= RISK_FREE_TRIGGER_PCT:
                    ...
                    log(f"✅ {sym} HIT RF +{pnl_pct:.2f}% - TRIGGERING INSTANT SCAN")
                    send_telegram(f"✅ {sym} {side} HIT RISK-FREE!\nEntry {entry:.4f} → Now {curr:.4f}\nPnL +{pnl_pct:.2f}% (Lev x{LEVERAGE})\n🔍 Scanning instantly for next trade...")
                    # INSTANT SCAN AFTER RF - no wait for 4H
                    def delayed_scan():
                        time.sleep(10)  # wait 10 sec to avoid race
                        try:
                            scan()
                        except Exception as e:
                            log(f"Instant scan after RF err {e}")
...
def home():
    return f"V44.8 AUTO-RF LIVE Key={'YES' if apiKey else 'NO'} TG={'YES' if TELEGRAM_BOT_TOKEN else 'NO'} Bal:{state['balance']:.2f} Pos:{len(state['positions'])} Scans:{state['scan_count']} SigToday:{state['signals_today']} Last:{state['last_scan']} {datetime.now().isoformat()}"
...
def ping():
    ...
    return f"PONG V44.8 {datetime.now().isoformat()} Pos:{len(state['positions'])} Bal:{state['balance']}", 200
...
if __name__ == '__main__':
    ...
    log("V44.8 Started with RF tracker every 5min + Uptime /ping endpoint")
