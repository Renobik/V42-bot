...
import re
TELEGRAM_TOKEN = re.sub(r'\s+', '', os.getenv("TELEGRAM_TOKEN", ""))
_raw = os.getenv("CHAT_ID", "") or os.getenv("TELEGRAM_CHAT_ID", "") or ""
CHAT_ID = re.sub(r'\s+', '', str(_raw)) or "200"
for k,v in os.environ.items():
    vv=re.sub(r'\s+', '', str(v))
    if vv.isdigit() and vv.startswith("675"):
        CHAT_ID=vv; break
WEBHOOK_SECRET = re.sub(r'\s+', '', os.getenv("WEBHOOK_SECRET", "Ogadagidi")) or "Ogadagidi"
...
def tg_send(msg, buttons=None, reply_chat=None):
    if not TELEGRAM_TOKEN:
        print(f"NO TOKEN - cannot send: {msg[:200]}")
        return
    try:
...
def scanner_loop():
    ...
    # QUIET MODE: Only 1 startup message, no spam
    tg_send(f"🏹 V44.20 HUNTER 24/7 LIVE Quiet Mode\nTop{SCAN_COUNT} | A+++/A+ Only | No Spam\n/status to check")
    while True:
        try:
            top=bitget_top200()
            update_positions()
            ...
            scanned=0
            for t in top:
                if len(state["positions"])>=1 or len(state["pending"])>=2: break # ULTRA QUIET: Max 2 pendings
                ...
                if res and res["ob"]:
                    ...
                    # ULTRA QUIET: Only A+++ (Sweep AND FVG AND Vol) - no A+
                    is_aplus = res["sweep_valid"] and res["fvg"] is not None
                    if is_aplus:
            ...
            # QUIET MODE: Removed spammy Checked message
            # Only logs to console, not Telegram
            if scanned>0:
                print(f"Checked={scanned}/{SCAN_COUNT} Pend={len(state['pending'])} Pos={len(state['positions'])}")
            time.sleep(300)  # ULTRA QUIET: Check every 5 min, not 60 sec
        except Exception as e:
