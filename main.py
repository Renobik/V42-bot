...
"""
V44.6 SENSITIVE MODE - More signals, same webhook, paper trading + reports
"""
import ccxt
import pandas as pd
import time
    ...
    try:
        ...
        for sym in top[:SCAN_TOP_N]:
            ...
            if side:
                ...
                if can_now:
                    state["positions"][sym] = {
                        "entry": 0, "side": side, "risk_free": False,
...
def home():
    return f"V44.6 SENSITIVE LIVE Key={'YES' if apiKey else 'NO'} TG={'YES' if TELEGRAM_BOT_TOKEN else 'NO'} Bal:{state['balance']:.2f} Pos:{len(state['positions'])} Scans:{state['scan_count']} SigToday:{state['signals_today']} Last:{state['last_scan']} {datetime.now().isoformat()}"
