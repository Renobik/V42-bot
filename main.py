...
"""
V44.8.1 INSTANT SCAN AFTER RF - Fixed & Stable
- Sensitive: 1H EMA20 + 15m Breakout
- Auto RF at +2% -> instant scan
- TP +6% / SL -4% -> instant scan
- Webhook + /ping for UptimeRobot
"""
import ccxt
import pandas as pd
import time
...
def get_top100():
    try:
        ...
        log(f"Top100: {len(top)} Top={top[0] if top else 'none'}")
        return top
    except Exception as e:
        log(f"Top100 err {e}")
...
def scan():
    ...
    try:
        ...
        if state["scan_count"] == 1 or state["scan_count"] % 6 == 0:
            send_telegram(f"✅ V44.8.1 LIVE! Checked={checked} Signals={signals} Today={state['signals_today']} Bal=${state['balance']:.2f} Pos={len(state['positions'])}")
        elif signals == 0:
        ...
        try:
            ...
            for sym, pos in list(state["positions"].items()):
                ...
                if not pos.get('risk_free') and pnl_pct >= RISK_FREE_TRIGGER_PCT:
                    ...
                    log(f"✅ {sym} HIT RF +{pnl_pct:.2f}% -> INSTANT SCAN")
...
def home():
    return f"V44.8.1 LIVE Bal:{state['balance']:.2f} Pos:{len(state['positions'])} Scans:{state['scan_count']} {datetime.now().isoformat()}"

@app.route('/ping', methods=['GET'])
@app.route('/health', methods=['GET'])
def ping():
    return f"PONG V44.8.1 {datetime.now().isoformat()} Pos:{len(state['positions'])}", 200
    ...
    try:
        ...
        if text.startswith('/status'):
            ...
            if state["positions"]:
                ...
                send_telegram(f"✅ Forced {len(state['positions'])} to RF -> instant scan 5 sec...")
                def forced_scan():
                    time.sleep(5)
                    try:
                        scan()
...
def set_webhook():
    ...
    try:
        ...
        send_telegram(f"🤖 V44.8.1 Started! Top {SCAN_TOP_N} every 4H + INSTANT after RF\nRF +2% -> instant scan\nSend /status /scan /rf")
    except Exception as e:
        log(f"Webhook err {e}")
