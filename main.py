...
# Support multiple env names - Render screenshot shows TEL... duplicates
CHAT_ID = os.getenv("CHAT_ID", "") or os.getenv("TELEGRAM_CHAT_ID", "") or os.getenv("TELEGRAM_CHAT", "") or os.getenv("CHAT", "") or "200"
# Find 675... value from screenshot
for k,v in os.environ.items():
    if v and v.startswith("675") and k.startswith("TEL"):
        CHAT_ID = v
        break
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "Ogadagidi")
...
def tg_send(msg, buttons=None):
    if not TELEGRAM_TOKEN: print(msg[:600]); return
    try:
...
def webhook():
    ...
    if "status" in txt or "/status" in txt:
        ...
        if len(parts)>=2:
            ...
            if not cur: tg_send(f"❌ No price {raw_sym}")
            else:
        ...
        if len(parts)>=2:
            ...
            save_state(state); tg_send(f"❌ Closed {target}")
        else:
