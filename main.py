...
def scanner_loop():
    ...
    # MEDIUM MODE: 3-5 signals/day high quality
    tg_send(f"🏹 V44.20 HUNTER 24/7 LIVE Medium Mode\nTop{SCAN_COUNT} | A+++/A+ (Sweep OR FVG) | 3-5/day\n/status to check")
    while True:
        try:
            top=bitget_top200()
            update_positions()
            ...
            scanned=0
            for t in top:
                if len(state["positions"])>=1 or len(state["pending"])>=3: break # MEDIUM: Max 3 pendings = 3-5 signals/day
                ...
                if res and res["ob"]:
                    ...
                    # MEDIUM: A+++ or A+ (Sweep OR FVG) = 3-5/day still high quality with HTF+Vol+OB50%
            ...
            # MEDIUM MODE: No spam Checked
            if scanned>0:
                print(f"Checked={scanned}/{SCAN_COUNT} Pend={len(state['pending'])} Pos={len(state['positions'])}")
            time.sleep(180)  # MEDIUM: Check every 3 min = 3-5 signals/day
        except Exception as e:
