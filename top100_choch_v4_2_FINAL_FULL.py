"""
V4.2 - Top100 4H/1H CHoCH Futures 5x Compound 5% - BYBIT VERSION
Fixed Binance IP ban on Render -> Now uses Bybit
Same logic, cleaner state, 5% risk locked, auto Top100 refresh, telegram_offset fix
"""
import ccxt, os, json, time, math, requests, io, threading
...
# --- FLASK FOR RENDER FREE PLAN (keeps service live) ---
app = Flask(__name__)
@app.route('/')
def home():
    return "V4.2 BYBIT Top100 CHoCH Live - OK", 200
def run_flask():
...
VOL_MULT_4H = 1.2
VOL_MULT_1H = 1.2
PULLBACK_DIST_PCT = 0.03  # within 3% of 4H 50% OB

STATE_FILE = "v4_2_state.json"
SIM_FILE = "v4_2_sim_balance.json"
OFFSET_FILE = "telegram_offset.json"

BYBIT_API_KEY = os.getenv("BYBIT_API_KEY", os.getenv("BINANCE_API_KEY",""))
BYBIT_API_SECRET = os.getenv("BYBIT_API_SECRET", os.getenv("BINANCE_API_SECRET",""))
...
def get_top100_symbols_bybit(exchange):
    # Try CMC first if available
    if CMC_API_KEY:
        try:
            ...
            data=r.json()
            cmc_syms=[f"{c['symbol']}/USDT:USDT" for c in data["data"]]
            markets=exchange.load_markets()
            filtered=[s for s in cmc_syms if s in markets and "UP" not in s and "DOWN" not in s and "BULL" not in s and "BEAR" not in s]
            if len(filtered)>=30:
                print(f"CMC Top100 loaded: {len(filtered)}", flush=True)
                return filtered[:100]
        except Exception as e:
            print(f"CMC error {e} fallback to Bybit", flush=True)
    # Bybit Top100 by quoteVolume
    try:
        tickers=exchange.fetch_tickers()
        usdt_perp=[(s,t.get("quoteVolume") or 0) for s,t in tickers.items() if s.endswith("USDT:USDT")]
        ...
        if not top:
            top = [s for s in exchange.symbols if s.endswith("USDT:USDT")][:100]
        print(f"Bybit Top100 by volume: {len(top)}", flush=True)
        return top
    except Exception as e:
        print(f"Top100 fetch error bybit {e}", flush=True)
        return ["BTC/USDT:USDT","ETH/USDT:USDT","SOL/USDT:USDT","BNB/USDT:USDT","XRP/USDT:USDT","DOGE/USDT:USDT","ADA/USDT:USDT","AVAX/USDT:USDT","LINK/USDT:USDT","DOT/USDT:USDT"]
...
def fetch_df(exchange, symbol, tf, limit=200):
    try:
        ...
        time.sleep(0.4)  # rate limit protection for Bybit
        return df
    except Exception as e:
        # print(f"fetch {symbol} {tf} err {e}")
        return None

def calc_atr(df, period=14):
    return (df["high"]-df["low"]).rolling(period).mean().iloc[-1]

def detect_choch(df, swing_len, vol_mult=1.2):
    ...
    vol_ok=df["vol"].iloc[-1] > vol_ma*vol_mult
...
def check_execution(bias_4h, df_1h, price_now):
    choch_1h=detect_choch(df_1h, SWING_LEN_1H, VOL_MULT_1H)
...
def build_chart(df_4h, df_1h, symbol, bias, exec_info):
    ...
    for ax, df, title in [(axes[0], df_4h, f"{symbol} 4H BIAS"), (axes[1], df_1h, f"{symbol} 1H EXEC - {exec_info['type']}")]:
        ...
        if title.startswith(f"{symbol} 4H"):
    ...
    try:
        ...
        for upd in r.get("result",[]):
            ...
            if cmd=="/balance":
                ...
                send_tg("🔍 Forced scan starting Top100 Bybit...")
                return True
            elif cmd=="/help":
...
def main():
    print(f"=== V4.2 BYBIT Top100 4H->1H 5% Risk {LEVERAGE}x DRY_RUN={DRY_RUN} ===", flush=True)
    exchange=ccxt.bybit({
    ...
    try: exchange.load_markets()
        ...
        print(f"load_markets err {e}", flush=True)

    symbols=get_top100_symbols_bybit(exchange)
    ...
    while True:
        ...
        # Refresh Top100 every 1 hour
        if time.time() - last_top100_refresh > 3600:
            symbols=get_top100_symbols_bybit(exchange)
            last_top100_refresh = time.time()
        ...
        for sym in symbols:
            ...
            bias=detect_choch(df_4h, SWING_LEN_4H, VOL_MULT_4H)
            ...
            if not DRY_RUN:
                try:
                    ...
                    exchange.create_order(sym,"limit",side,qty,entry)
                    exchange.create_order(sym,"STOP_MARKET",opp,qty,None, params={"stopPrice":sl,"reduceOnly":True})
                    rr2=2; rr3=3
                    if bias["dir"]=="long":
                    ...
                    print(f"Order err {sym} {e}", flush=True)
                    continue
        ...
        save_json(STATE_FILE, state)
        print(f"Scan done. Signals {len(signals)}. Sleep {SCAN_INTERVAL_MIN}min", flush=True)
